...
final class BlockTypeTest extends TestCase
{
    public function testAllBlockTypesExist(): void
    {
        ...
        $this->assertContains('real_widget', $values);
        $this->assertContains('replacements', $values);
        $this->assertContains('tabbed', $values);
        $this->assertContains('accordion', $values);
        $this->assertContains('login_state', $values);
        $this->assertContains('cart', $values);
        $this->assertContains('dynamic_html', $values);
    }
    ...
    public function testAllCasesHaveIcons(): void
    {
        foreach (BlockType::cases() as $type) {
            ...
            $this->assertStringStartsWith('dashicons-', $type->icon());
        }
    }

    public function testCount(): void
    {
        $this->assertCount(21, BlockType::cases());
    }

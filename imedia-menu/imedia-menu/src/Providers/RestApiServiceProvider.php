...
final class RestApiServiceProvider implements ServiceProvider {
	...
	private function registerToggleBarRoutes(): void {
		register_rest_route(
			'imedia-menu/v1',
			'/toggle-bar/(?P<slug>[a-z0-9_-]+)',
			array(
				'methods'             => \WP_REST_Server::READABLE,
				'callback'            => array( $this, 'getToggleBar' ),
				'permission_callback' => array( $this, 'checkPermission' ),
				'args'                => array(
					'slug' => array(
						'required'          => true,
		...
		register_rest_route(
			...
			array(
				'methods'             => \WP_REST_Server::CREATABLE,
				'callback'            => array( $this, 'saveToggleBar' ),
				'permission_callback' => array( $this, 'checkPermission' ),
				'args'                => array(
					'slug' => array(
						'required'          => true,
		...
		register_rest_route(
			...
			array(
				'methods'             => \WP_REST_Server::DELETABLE,
				'callback'            => array( $this, 'deleteToggleBar' ),
				'permission_callback' => array( $this, 'checkPermission' ),
				'args'                => array(
					'slug' => array(
						'required'          => true,
	...
	public function saveLocationOverrides( \WP_REST_Request $request ): \WP_REST_Response {
		...
		$overrides = $this->sanitizeLocationOverrides( $overrides );
	...
	private function sanitizeLocationOverrides( array $overrides ): array {
		$clean = array();

		if ( isset( $overrides['sticky'] ) ) {
			$clean['sticky'] = (bool) $overrides['sticky'];
		}

		if ( isset( $overrides['sticky_desktop'] ) ) {
			$clean['sticky_desktop'] = (bool) $overrides['sticky_desktop'];
		}

		if ( isset( $overrides['sticky_mobile'] ) ) {
			$clean['sticky_mobile'] = (bool) $overrides['sticky_mobile'];
		}

		if ( isset( $overrides['sticky_opacity'] ) ) {
			$opacity                 = (float) $overrides['sticky_opacity'];
			$clean['sticky_opacity'] = min( 1.0, max( 0.2, $opacity ) );
		}

		if ( isset( $overrides['sticky_offset'] ) ) {
			$offset                 = (int) $overrides['sticky_offset'];
			$clean['sticky_offset'] = min( 500, max( 0, $offset ) );
		}

		if ( isset( $overrides['sticky_expand'] ) ) {
			$clean['sticky_expand'] = (bool) $overrides['sticky_expand'];
		}

		if ( isset( $overrides['sticky_expand_mobile'] ) ) {
			$clean['sticky_expand_mobile'] = (bool) $overrides['sticky_expand_mobile'];
		}

		if ( isset( $overrides['sticky_hide_until_scroll_up'] ) ) {
			$clean['sticky_hide_until_scroll_up'] = (bool) $overrides['sticky_hide_until_scroll_up'];
		}

		if ( isset( $overrides['sticky_hide_until_scroll_up_tolerance'] ) ) {
			$tol = (int) $overrides['sticky_hide_until_scroll_up_tolerance'];
			$clean['sticky_hide_until_scroll_up_tolerance'] = min( 200, max( 0, $tol ) );
		}

		if ( isset( $overrides['sticky_hide_until_scroll_up_offset'] ) ) {
			$off = (int) $overrides['sticky_hide_until_scroll_up_offset'];
			$clean['sticky_hide_until_scroll_up_offset'] = min( 500, max( 0, $off ) );
		}

		if ( isset( $overrides['transparent_mode'] ) ) {
			$clean['transparent_mode'] = (bool) $overrides['transparent_mode'];
		}

		if ( isset( $overrides['trigger_type'] ) && in_array( $overrides['trigger_type'], array( 'hover', 'click', 'hover_click' ), true ) ) {
			$clean['trigger_type'] = $overrides['trigger_type'];
		}

		if ( isset( $overrides['orientation'] ) && in_array( $overrides['orientation'], array( 'horizontal', 'vertical', 'accordion' ), true ) ) {
			$clean['orientation'] = $overrides['orientation'];
		}

		if ( isset( $overrides['overlay'] ) && in_array( $overrides['overlay'], array( 'off', 'desktop', 'mobile', 'both' ), true ) ) {
			$clean['overlay'] = $overrides['overlay'];
		}

		if ( isset( $overrides['overlay_color'] ) ) {
			$clean['overlay_color'] = sanitize_text_field( $overrides['overlay_color'] );
		}
		...
		foreach ( $passThrough as $key ) {
			if ( isset( $overrides[ $key ] ) ) {
				$clean[ $key ] = is_string( $overrides[ $key ] )
					? sanitize_text_field( $overrides[ $key ] )
					: $overrides[ $key ];
			}
		}
	...
	public function getIcons(): \WP_REST_Response {
		...
		if ( in_array( 'fontawesome5', $enabledProviders, true ) ) {
			$manager->register( new \IMedia\Menu\Icons\Providers\FontAwesome5Provider() );
		}

		if ( in_array( 'fontawesome6', $enabledProviders, true ) ) {
			$manager->register( new \IMedia\Menu\Icons\Providers\FontAwesome6Provider() );
		}

		if ( in_array( 'genericons', $enabledProviders, true ) ) {
			$manager->register( new \IMedia\Menu\Icons\Providers\GenericonsProvider() );
		}

		if ( in_array( 'bootstrap_icons', $enabledProviders, true ) ) {
			$manager->register( new \IMedia\Menu\Icons\Providers\BootstrapIconsProvider() );
		}
	...
	public function getToggleBar( \WP_REST_Request $request ): \WP_REST_Response {
		$slug = sanitize_title( $request->get_param( 'slug' ) );
		...
		$repository = new \IMedia\Menu\Frontend\ToggleBar\ToggleBarRepository();
		$blocks     = $repository->get( $slug );
	...
	public function saveToggleBar( \WP_REST_Request $request ): \WP_REST_Response {
		$slug = sanitize_title( $request->get_param( 'slug' ) );
		...
		$body   = $request->get_json_params();
		$blocks = is_array( $body ) && isset( $body['blocks'] ) && is_array( $body['blocks'] )
			? $body['blocks']
			: ( is_array( $body ) ? $body : array() );

		$repository = new \IMedia\Menu\Frontend\ToggleBar\ToggleBarRepository();
		$repository->save( $slug, $blocks );

		do_action( 'imm_toggle_bar_saved', $slug, $blocks );
	...
	public function deleteToggleBar( \WP_REST_Request $request ): \WP_REST_Response {
		$slug = sanitize_title( $request->get_param( 'slug' ) );
		...
		do_action( 'imm_toggle_bar_deleted', $slug );

...
final class PanelRepository {
	...
	public function save( int $menuItemId, int $menuId, array $data ): bool {
		...
		$layoutType = $data['layout_type'] ?? 'columns';
		if ( ! in_array( $layoutType, array( 'columns', 'grid', 'flyout' ), true ) ) {
			$layoutType = 'columns';
		}
	...
	public function hasEnabledGridPanels(): bool {
		global $wpdb;

		$count = (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$this->table} WHERE is_enabled = 1 AND layout_type = %s",
				'grid'
			)
		);

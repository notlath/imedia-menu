...
final class MenuWalker extends \Walker_Nav_Menu
{
    ...
    public function start_el(string &$output, object $item, int $depth = 0, ?array $args = null, int $id = 0): void
    {
        ...
        $hasKids   = in_array('menu-item-has-children', $item->classes ?? [], true);
        ...
        $this->renderLink($output, $item, $depth, $hasMega, $isMega);
    }
    ...
    private function getItemClasses(object $item, int $depth): array
    {
        ...
        $currentItemClasses = $item->classes ?? [];
    ...
    private function renderLink(string &$output, object $item, int $depth, bool $hasMega, bool $isMega): void
    {
        ...
        $hasPopup  = $isMega || $hasMega;
        $attrs     = ['class' => 'imm-link'];
